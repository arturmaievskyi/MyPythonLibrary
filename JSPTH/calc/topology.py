...
class EuclideanTopology(Topology):
    ...
    Open sets are unions of open intervals (in 1D) or open balls (in higher dimensions).
    """
    ...
    def __init__(self, dimension=1):
        ...
        # For simplicity, we'll represent the space with discrete points
        self.base_set = set(range(-10, 11))  # Representative finite set
        self.open_sets = self._generate_standard_topology()
        self.closed_sets = []
        self._compute_closed_sets()

    def _generate_standard_topology(self):
        """Generate open intervals for 1D Euclidean topology."""
        open_sets = [set()]  # Empty set
        open_sets.append(self.base_set)  # Full set
        
        # Add some open intervals
        for a in range(-10, 10):
            for b in range(a + 2, 11):
                open_interval = set(range(a + 1, b))
                if open_interval:
                    open_sets.append(open_interval)
